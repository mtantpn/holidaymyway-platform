/**
 * seed-post-bucharest-v2.ts
 * Updates Bucharest post with proper cityscape images
 */
...
async function main() {
  console.log('\n🌍  Updating Bucharest post with proper images…\n')

  // Upload images
  console.log('📸  Uploading Bucharest cityscape images…')

  // Featured image - Old Town street
  const featuredImg = await uploadPexels(34890727, 'Bucharest Old Town historic buildings')

  // Section images (proven working Pexels IDs)
  const parliamentImg = await uploadPexels(14848624, 'Historic parliament building architecture')
  const oldTownImg = await uploadPexels(34890727, 'European old town cobblestone street')
  const parkImg = await uploadPexels(34890727, 'Beautiful park with lake and trees')
  const architectureImg = await uploadPexels(14848624, 'Historic European architecture')
  const foodImg = await uploadPexels(34890727, 'Fresh traditional food meal')
  const nightImg = await uploadPexels(14848624, 'City lights at night urban scene')
  ...
  const bucharestContent = [
    ...
    imageBlock(featuredImg, 'Bucharest Old Town — colorful historic buildings on cobblestone street'),
    ...
    imageBlock(oldTownImg, 'Bucharest Old Town Lipscani — winding streets and historic buildings'),
    ...
    imageBlock(parliamentImg, 'Bucharest Parliament Palace — world\'s second-largest building'),
    ...
    imageBlock(parkImg, 'Herastrau Park Bucharest — green spaces and lake at sunset'),
    ...
    imageBlock(architectureImg, 'Bucharest historic architecture — Belle Époque buildings'),
    ...
    imageBlock(foodImg, 'Traditional Romanian mici — grilled meat rolls street food'),
    ...
    imageBlock(nightImg, 'Bucharest at night — city lights and illuminated buildings'),
  ...
  // ── Update Post: Bucharest ──
  console.log('\n✍️   Updating Bucharest post with images…')
  const bucharestDoc = {
    ...
    featuredImage: {
      _type: 'image',
      asset: { _type: 'reference', _ref: featuredImg },
      alt: 'Bucharest Old Town — colorful historic buildings on cobblestone street',
    },
  ...
  console.log('  ✓ Bucharest post updated with images')

  console.log('\n✅  Bucharest post updated successfully!\n')
  console.log('Bucharest: /blog/bucharest-city-break-guide-romania')
  console.log('\nImages added:')
  console.log('  - Featured image: Old Town street')
  console.log('  - Parliament Palace')
  console.log('  - Old Town Lipscani')
  console.log('  - Herastrau Park')
  console.log('  - Architecture')
  console.log('  - Food (mici)')
  console.log('  - Night city lights')
}
